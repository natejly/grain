...
# parks — assignment, the inbox, digests, per-thread visibility, document hunks —
# and none of that has anything to test if nothing parks. So the baseline here is
# the strict mode, and a test about a bypass says so by setting the mode itself.
#
# Pinned rather than left to the shipped default so the two can never be confused
# again: `test_approval_defaults.py` asserts the product default explicitly, which
# is the honest place for that claim to live. A suite that got its parking for
# free from a default would go silently green the day the default changed.
