...
def replace_content(
    ...
    *,
    ...
    # Python attribute, not a column, hence the mypy ignore — the route reads
    # it back with getattr). The PUT route used to recompute the head with a
    # second query after the commit, and a save landing in that window handed
    # the caller someone else's head as their new base — their next save then
    # passed the precondition over content they never saw.
    document.saved_head_version_id = snapshot.id  # type: ignore[attr-defined]
    return document
