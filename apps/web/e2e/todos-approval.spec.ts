...
test("a write parks under ask_writes, runs under the bypass, and a deny survives it", async ({
  ...
  // trail rather than a standing warning: a banner rendered on every thread in
  // the product would be furniture, and furniture is not read on the day it
  // finally has something to say. It appears below, once a call has actually
  // gone through unreviewed.
