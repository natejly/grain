...
describe("chordEligible", () => {
  ...
  it("accepts a bare letter aimed at the page", () => {
    expect(chordEligible({ ...bare, target: document.body })).toBe(true);
    expect(chordEligible({ ...bare, target: null })).toBe(true);
  });

  it("refuses modified keys — ⌘G and friends belong to the browser", () => {
    expect(chordEligible({ ...bare, metaKey: true, target: document.body })).toBe(false);
    expect(chordEligible({ ...bare, ctrlKey: true, target: document.body })).toBe(false);
    expect(chordEligible({ ...bare, altKey: true, target: document.body })).toBe(false);
  });

  it("refuses non-letter keys", () => {
    expect(chordEligible({ ...bare, key: "Escape", target: document.body })).toBe(false);
  });

  it("refuses keys typed into a field — chords must not fire mid-sentence", () => {
    for (const tag of ["input", "textarea", "select"] as const) {
      expect(chordEligible({ ...bare, target: document.createElement(tag) })).toBe(false);
    }
    const editable = document.createElement("div");
    // jsdom does not compute isContentEditable from the attribute alone.
    Object.defineProperty(editable, "isContentEditable", { value: true });
    expect(chordEligible({ ...bare, target: editable })).toBe(false);
    expect(chordEligible({ ...bare, target: document.createElement("div") })).toBe(true);
  });
});
